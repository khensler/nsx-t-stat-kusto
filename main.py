...
class NSXTTier0:
    def __init__(self, path, name, connection):
        ...
        self.local_services = getT0LocaleServices(self.connection,self.path)
        self.interfaces = getT0Interfaces(self)
...
class NSXEdgeNode:
    ...
    def getCPUStats(self):
        nodecpu = json.loads(getAPIResults(nsxtConnection=self.connection,uri= "/api/v1/transport-nodes/"+self.id+"/node/services/dataplane/cpu-stats",policy=False))
...
def getT0Interfaces(NSXTTier0):
    interfaces = []
    results = json.loads(getAPIResults(nsxtConnection=NSXTTier0.connection,uri= NSXTTier0.local_services+"/interfaces"))
...
def getT0(nsxtConnection, id):
    results = json.loads(getAPIResults(nsxtConnection=nsxtConnection, uri="/infra/tier-0s/"+id))
    return NSXTTier0(path = results['path'], name = results['display_name'], connection=nsxtConnection)

def getT0s(nsxtConnection):
    T0s = []
    results = json.loads(getAPIResults(nsxtConnection=nsxtConnection, uri="/infra/tier-0s"))
    for t0 in results['results']:
        T0s.append(getT0(nsxtConnection=nsxtConnection, id=t0['display_name']))
    return T0s

def getT0LocaleServices(nsxtConnection,path):
    tier0locales = json.loads(getAPIResults(nsxtConnection=nsxtConnection, uri=path+"/locale-services"))  
    return tier0locales['results'][0]['path']

def getInterfacesStats(tier0interfaces):
    frames =[]
    for tier0interface in tier0interfaces:
        interfaceStats = getInterfaceStats(tier0interface)
        frames.append(interfaceStats)
    return pandas.concat(frames)

def getInterfaceStats(tier0interface):
    ...
    tier0interfacestats = json.loads(getAPIResults(nsxtConnection=tier0interface.connection, uri=statsuri))
...
def getEdgeNodes(nsxtConnection):
    nodes = []
    results = json.loads(getAPIResults(nsxtConnection=nsxtConnection, uri="/api/v1/transport-nodes?node_types=EdgeNode", policy=False))['results']
    for node in results:
...
def getAPIResults(nsxtConnection, uri,json_body=None, policy = True):
    if (policy == True):
